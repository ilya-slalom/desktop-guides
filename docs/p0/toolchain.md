...
The host already had Windows App Runtime and WebView2 installed. A clean Windows 11 x64 VM must still establish the precise missing-framework install error, missing-WebView2 startup behavior, and recovery after offline installers. Windows 10 x64 remains deferred and untested. Native ARM64 CI installed and launched the package, then passed 23 Core tests and [14/14 reader fixtures](evidence/arm64-ci/ui-suite/suite.json). The ARM64 UI test used ValuePattern for the fixture password; it did not test physical keyboard entry or offline operation on ARM64.
