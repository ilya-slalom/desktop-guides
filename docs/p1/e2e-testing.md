...
| --- | --- | --- |
| Shell smoke | Fresh empty Library, Library/Game/Reader/Settings routes, rapid Game/Guide → Settings selections and Back, stale Resume, no P0 fixture controls, and a positive UI-accepted redirected activation. Launch a new window while an old guide write is blocked; verify the new window waits for the library lease and then shows the distinct guide saved by the old window. Pause a second launch after it selects the old instance, and pause a redirected callback after it reaches the UI queue; close the old window in each case and verify the launch takes over. Obtain test-owned process IDs from the interactive launch helper, then validate the executable path, start time, and session before UI checks or cleanup. Reader is still a placeholder. | T11.1, TR11.1 |
