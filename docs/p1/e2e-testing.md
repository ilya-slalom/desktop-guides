...
| --- | --- | --- |
| Shell smoke | Fresh empty Library, Library/Game/Reader/Settings routes, rapid Game/Guide → Settings selections and Back, stale Resume, no P0 fixture controls, and a positive redirected-activation acknowledgment. Launch a new window while an old guide write is blocked; verify the new window waits for the library lease and then shows the distinct guide saved by the old window. Pause a second launch after it selects the old instance, close that instance, and verify the launch takes over. Bind UI observations and cleanup to test-owned process IDs and the interactive session. Reader is still a placeholder. | T11.1, TR11.1 |
