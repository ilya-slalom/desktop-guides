...
| P1 M0 | Implemented on `feat/p1-m0` for review; not yet merged. | [M0 results](p1/results.md) record storage contracts/schema, managed path tests, typed reader contract, locator codecs, and the PDF decision. Windows 11 x64 headless tests and the diagnostic WinUI PDF UIA run passed; CI is pending. |
