...
| P1 M1 | T03.2 merged through [PR #4](https://github.com/ilya-slalom/desktop-guides/pull/4) on 26 September 2026, merge commit `73ae32db8dee509db121f00118a17e460fe60404`. T15.2 is implemented for review in [PR #5](https://github.com/ilya-slalom/desktop-guides/pull/5); T11.1, T11.3, and T17.1 remain open. | The [M1 result](p1/results.md) records v1→v2 upgrade and recovery, then startup FileOperations reconciliation with exact owned-path cleanup. Locked Windows 11 x64 tests passed 61 Core and 41 Infrastructure cases after the uppercase-ID review fix. Prior PR CI passed x64 and native ARM64 headless tests, both unsigned packages, and the installed ARM64 P0 fixture regression; current-head results are in PR checks. The production shell remains to be checked. |
