...
Reject filesystem links at `library.sqlite` and its parent paths before
opening the database, including later repository opens. For each supported
version, compare app-owned table and index definitions with a reference
database built from that version's migration scripts. This checks columns,
constraints, and index definitions that object-name checks miss. Exact
matching deliberately rejects manual schema alterations while preserving
their data for recovery; SQLite's internal `sqlite_*` objects are excluded.
